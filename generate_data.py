...
if __name__ == '__main__':
    ...
    female_model_path = './models/female_model.pkl'
    male_model_path = './models/male_model.pkl'

    model = load_model( female_model_path )           # the loaded model object is a 'chumpy' object, check https://github.com/mattloper/chumpy for details
    ...
    for n in range(5000):
      ...
      outmesh_path = join( outmesh_dir, 'female_sample' + str(n) + '.obj')
      write_simple_obj( mesh_v=model.r, mesh_f=model.f, filepath=outmesh_path )

      print('output mesh saved to: ', outmesh_path) 

      model = load_model( male_model_path )           # the loaded model object is a 'chumpy' object, check https://github.com/mattloper/chumpy for details
    ...
    for n in range(5000):
      ...
      outmesh_dir = './output'
      safe_mkdir( outmesh_dir )
      outmesh_path = join( outmesh_dir, 'male_sample' + str(n) + '.obj')
      write_simple_obj( mesh_v=model.r, mesh_f=model.f, filepath=outmesh_path )
